...
st.subheader('Remote work ratio plot : ')
...
sel_country=salary[salary['employee_residence']==select_loc]
df3 = sel_country.groupby('employee_residence', as_index=False)['salary_in_usd'].mean()
df3.sort_values('salary_in_usd', axis = 0, ascending = False, inplace = True)
df3 = df2.head(10)
df3
st.bar_chart(data=df3, x='employee_residence', y='salary_in_usd', width=0, height=0, use_container_width=True)
...
st.bar_chart(data=df1, x='job_title', y='salary_in_usd', width=0, height=0, use_container_width=True)
